...
RandomisedGreedyTSP::RandomisedGreedyTSP(std::vector<City> cities, Car car) : AbstractTSP(cities, car) {
    int n = cities.size();
...
void RandomisedGreedyTSP::solveTSP() {
    int n = cities.size();
    float k = 1.02; //a set coefficient of the diffrence in fuel consumption based on elevation. https://www.quora.com/How-many-feet-of-elevation-increase-is-equivalent-in-fuel-usage-to-driving-an-extra-mile-with-no-elevation-change
    std::vector<std::vector<double>> fuelConsumptionMatrix(n, std::vector<double>(n)); //A matrix that will be filled with data representing the fuel consumption between two cities
    ...
    // startingCity = 4; //ability to start at a given city
    ...
    std::cout << solutionTSP[n] << "\nRandomisedGreedy: The consumed fuel using this route was: " << consumed_fuel << std::endl;
    std::cout << "Time difference = " << std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count() << "[ns]" << std::endl;

    nlohmann::json jsonData;
    jsonData["route"] = solutionTSP;

    std::ofstream outputFile("./solution/solutionRandomised.json");
